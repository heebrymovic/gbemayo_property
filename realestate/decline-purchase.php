...
if (isset($_GET['property_id']) && !empty($_GET['property_id']) && isset($_GET['property_payment_id']) && !empty($_GET['property_payment_id']) && isset($_GET['client_id']) && !empty($_GET['client_id']) && $session_logged_company_privilege_id == 1) {
	...
	$update_property_buy = update_property_payment($property_payment_id, $property_id,  $client_id, "cancelled");
	

	if ($update_property_buy) {
		
		echo "<script>
		alert('Property Purchase Payment Successfully Declined');
		window.location=history.back();
		</script>";
		
	}
	else{
		echo "<script>alert('Oops!!, Error occur while trying to decline property purchase payment, please try again later');
		window.location=history.back();
		</script>";
	}
